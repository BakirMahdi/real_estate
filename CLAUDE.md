...
- **`scrapers/`** (`tayara.py`, `mubawab.py`, `expat.py`) — each exposes `iter_scrape_phases(progress_callback, cancel_event, known_ad_ids)`, a generator yielding `(phase, items)` per phase. Phases are `rent`, `sale`, `land`. Ads already in the DB (`known_ad_ids`) are **not re-downloaded** unless `SCRAPER_REFRESH_ALL=1` — the biggest re-run speedup.
